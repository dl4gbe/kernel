...
abstract class cls_base_contract_template_item extends cls_datatable_base
{
...
{
	return array('freeunitsperyear','freeunitspermonth','freeunitsperweek','freeunitsperday','increaseperyear','increasepermonth','amountperyear','amountpermonth','amountperweek','amountperday','durationinmonths','maincontract','id_article','name','id');
}

public function get_table_select($delimiter = '"')
{
	return 'select freeunitsperyear as ' . $delimiter . 'contract_template_item.freeunitsperyear' . $delimiter . ',freeunitspermonth as ' . $delimiter . 'contract_template_item.freeunitspermonth' . $delimiter . ',freeunitsperweek as ' . $delimiter . 'contract_template_item.freeunitsperweek' . $delimiter . ',freeunitsperday as ' . $delimiter . 'contract_template_item.freeunitsperday' . $delimiter . ',increaseperyear as ' . $delimiter . 'contract_template_item.increaseperyear' . $delimiter . ',increasepermonth as ' . $delimiter . 'contract_template_item.increasepermonth' . $delimiter . ',amountperyear as ' . $delimiter . 'contract_template_item.amountperyear' . $delimiter . ',amountpermonth as ' . $delimiter . 'contract_template_item.amountpermonth' . $delimiter . ',amountperweek as ' . $delimiter . 'contract_template_item.amountperweek' . $delimiter . ',amountperday as ' . $delimiter . 'contract_template_item.amountperday' . $delimiter . ',durationinmonths as ' . $delimiter . 'contract_template_item.durationinmonths' . $delimiter . ',maincontract as ' . $delimiter . 'contract_template_item.maincontract' . $delimiter . ',id_article as ' . $delimiter . 'contract_template_item.id_article' . $delimiter . ',name as ' . $delimiter . 'contract_template_item.name' . $delimiter . ',id as ' . $delimiter . 'contract_template_item.id' . $delimiter . ' from contract_template_item';
}
...
public function save($db_manager,$application)
	{
		require_once('include/data/base_table_savers/cls_save_contract_template_item.php');
		return cls_save_contract_template_item::save_object($this,$db_manager,$application);
	}

public function save_array($data,$db_manager,$application)
	{
		require_once('include/data/base_table_savers/cls_save_contract_template_item.php');
		return cls_save_contract_template_item::save_array($data,$db_manager,$application);
	}
...
public function get_contract_template_items($db_manager,$application,$search_values,$limit = 0,$offset = 0,$use_key_value = false)
	{
		$result = null;
		if (empty($seach_values))
		{
			$result = $db_manager->search_for_records('contract_template_item',null,null,$limit,$offset);
		}
		else
		{
			$result = $db_manager->search_for_records('contract_template_item',$this->get_search_fields(),$seach_values,$limit,$offset);
		}
...
public function get_table_test_data($contract_template_items,$db_manager,$application,$use_key_value = false)
	{
...
public function get_multi_table_test_data($contract_template_items,$db_manager,$application)
	{
		$result = $db_manager->get_records_by_ids('table_test_data',$contract_template_items,'id_data');
		if (is_null($result)) return null;
		if (count($result) == 0) return null;
		require_once('include/data/table_factory/cls_table_factory.php');
		$data = array();
...
public function get_communication_reason($contract_template_items,$db_manager,$application,$use_key_value = false)
	{
...
public function get_multi_communication_reason($contract_template_items,$db_manager,$application)
	{
		$result = $db_manager->get_records_by_ids('communication_reason',$contract_template_items,'id_data');
		if (is_null($result)) return null;
		if (count($result) == 0) return null;
		require_once('include/data/table_factory/cls_table_factory.php');
		$data = array();
...
public function get_data_change($contract_template_items,$db_manager,$application,$use_key_value = false)
	{
...
public function get_multi_data_change($contract_template_items,$db_manager,$application)
	{
		$result = $db_manager->get_records_by_ids('data_change',$contract_template_items,'id_data');
		if (is_null($result)) return null;
		if (count($result) == 0) return null;
		require_once('include/data/table_factory/cls_table_factory.php');
		$data = array();
...
public function get_data_help($contract_template_items,$db_manager,$application,$use_key_value = false)
	{
...
public function get_multi_data_help($contract_template_items,$db_manager,$application)
	{
		$result = $db_manager->get_records_by_ids('data_help',$contract_template_items,'id_data');
		if (is_null($result)) return null;
		if (count($result) == 0) return null;
		require_once('include/data/table_factory/cls_table_factory.php');
		$data = array();
...
public function get_data_location($contract_template_items,$db_manager,$application,$use_key_value = false)
	{
...
public function get_multi_data_location($contract_template_items,$db_manager,$application)
	{
		$result = $db_manager->get_records_by_ids('data_location',$contract_template_items,'id_data');
		if (is_null($result)) return null;
		if (count($result) == 0) return null;
		require_once('include/data/table_factory/cls_table_factory.php');
		$data = array();
...
public function get_data_posting($contract_template_items,$db_manager,$application,$use_key_value = false)
	{
...
public function get_multi_data_posting($contract_template_items,$db_manager,$application)
	{
		$result = $db_manager->get_records_by_ids('data_posting',$contract_template_items,'id_data');
		if (is_null($result)) return null;
		if (count($result) == 0) return null;
		require_once('include/data/table_factory/cls_table_factory.php');
		$data = array();
...
public function get_offer_event($contract_template_items,$db_manager,$application,$use_key_value = false)
	{
...
public function get_multi_offer_event($contract_template_items,$db_manager,$application)
	{
		$result = $db_manager->get_records_by_ids('offer_event',$contract_template_items,'id_data');
		if (is_null($result)) return null;
		if (count($result) == 0) return null;
		require_once('include/data/table_factory/cls_table_factory.php');
		$data = array();
...
public function get_supplier_data($contract_template_items,$db_manager,$application,$use_key_value = false)
	{
...
public function get_multi_supplier_data($contract_template_items,$db_manager,$application)
	{
		$result = $db_manager->get_records_by_ids('supplier_data',$contract_template_items,'id_data');
		if (is_null($result)) return null;
		if (count($result) == 0) return null;
		require_once('include/data/table_factory/cls_table_factory.php');
		$data = array();
...
public function get_address($contract_template_items,$db_manager,$application,$use_key_value = false)
	{
...
public function get_multi_address($contract_template_items,$db_manager,$application)
	{
		$result = $db_manager->get_records_by_ids('address',$contract_template_items,'id_data');
		if (is_null($result)) return null;
		if (count($result) == 0) return null;
		require_once('include/data/table_factory/cls_table_factory.php');
		$data = array();
...
public function get_data_property($contract_template_items,$db_manager,$application,$use_key_value = false)
	{
...
public function get_multi_data_property($contract_template_items,$db_manager,$application)
	{
		$result = $db_manager->get_records_by_ids('data_property',$contract_template_items,'id_data');
		if (is_null($result)) return null;
		if (count($result) == 0) return null;
		require_once('include/data/table_factory/cls_table_factory.php');
		$data = array();
...
public function get_data_translation($contract_template_items,$db_manager,$application,$use_key_value = false)
	{
...
public function get_multi_data_translation($contract_template_items,$db_manager,$application)
	{
		$result = $db_manager->get_records_by_ids('data_translation',$contract_template_items,'id_data');
		if (is_null($result)) return null;
		if (count($result) == 0) return null;
		require_once('include/data/table_factory/cls_table_factory.php');
		$data = array();
...
public function get_dms($contract_template_items,$db_manager,$application,$use_key_value = false)
	{
...
public function get_multi_dms($contract_template_items,$db_manager,$application)
	{
		$result = $db_manager->get_records_by_ids('dms',$contract_template_items,'id_data');
		if (is_null($result)) return null;
		if (count($result) == 0) return null;
		require_once('include/data/table_factory/cls_table_factory.php');
		$data = array();
...
public function get_data_relation_id_data1($contract_template_items,$db_manager,$application,$use_key_value = false)
	{
...
public function get_multi_data_relation_id_data1($contract_template_items,$db_manager,$application)
	{
		$result = $db_manager->get_records_by_ids('data_relation',$contract_template_items,'id_data1');
		if (is_null($result)) return null;
		if (count($result) == 0) return null;
		require_once('include/data/table_factory/cls_table_factory.php');
		$data = array();
...
public function get_data_relation_id_data2($contract_template_items,$db_manager,$application,$use_key_value = false)
	{
...
public function get_multi_data_relation_id_data2($contract_template_items,$db_manager,$application)
	{
		$result = $db_manager->get_records_by_ids('data_relation',$contract_template_items,'id_data2');
		if (is_null($result)) return null;
		if (count($result) == 0) return null;
		require_once('include/data/table_factory/cls_table_factory.php');
		$data = array();
...
public function get_data_property_id_link_data($contract_template_items,$db_manager,$application,$use_key_value = false)
	{
...
public function get_multi_data_property_id_link_data($contract_template_items,$db_manager,$application)
	{
		$result = $db_manager->get_records_by_ids('data_property',$contract_template_items,'id_link_data');
		if (is_null($result)) return null;
		if (count($result) == 0) return null;
		require_once('include/data/table_factory/cls_table_factory.php');
		$data = array();
